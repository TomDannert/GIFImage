...
type
  ...
  TImageData = packed record
    ...
    Delay   : Integer;
    Size    : Integer;        // Size of image bits in Bytes
  ...
  private
    ...
    procedure InternalRender(AIndex: Integer; var ABitmap: TBitmap);
  protected
...
function NewImage(Width, Height: Integer; var Image: TImageData): Boolean;
begin
...
begin
  ...
  try
    ...
    RaiseImaging(SErrorCustomPalette, [APal, AEntries]);
  end;
end;

procedure FillMemoryCardinal(Data: Pointer; Size: Integer; Value: Cardinal);
{$IFDEF USE_ASM}
asm
  PUSH   EDI
  PUSH   EBX
...
begin
  for I := 0 to Size div 4 - 1 do
    PCardinalArray(Data)[I] := Value;
  case Size mod 4 of
    1: PByteArray(Data)[Size - 1] := TCardinalRec(Value).Bytes[0];
    2: PWordArray(Data)[Size div 2] := TCardinalRec(Value).Words[0];
    3:
      begin
        PWordArray(Data)[Size div 2 - 1] := TCardinalRec(Value).Words[0];
        PByteArray(Data)[Size - 1] := TCardinalRec(Value).Bytes[0];
      end;
  end;
end;
{$ENDIF}

procedure FillMemoryByte(Data: Pointer; Size: Integer; Value: Byte);
{$IFDEF USE_ASM}
asm
  PUSH   EDI
  MOV    EDI, EAX
...
procedure FillMemoryWord(Data: Pointer; Size: Integer; Value: Word);
{$IFDEF USE_ASM}
asm
  PUSH   EDI
  PUSH   EBX
...
procedure CopyPixel(Src, Dest: Pointer; BytesPerPixel: Integer);
begin
  case BytesPerPixel of
    1: PByte(Dest)^ := PByte(Src)^;
    2: PWord(Dest)^ := PWord(Src)^;
    3: PColor24Rec(Dest)^ := PColor24Rec(Src)^;
    4: PAlphaColorRec(Dest)^ := PAlphaColorRec(Src)^;
  end;
end;
...
procedure ConvertToPixel32(SrcPix: PByte; DestPix: PAlphaColorRec; SrcPalette: PPalette32);
begin
...
begin
  ...
  begin
    Dst := @PAlphaColorRecArray(Image.Bits)[(Top + Y) * Image.Width + Left];
...
procedure CopyImageToBitmap(const Image: TImageData; Bitmap: TBitmap; DstX, DstY: Integer);
var
  SrcX, SrcY, Width, Height : Integer;
  X, Y, SrcWidthBytes, MoveBytes: Integer;
  DstPtr: PAlphaColor;
  SrcPtr: PByte;
  Data : TBitmapData;
begin
...
begin
  ...
  try
    ...
    begin
      DstPtr := PAlphaColorRec(Data.GetScanLine(Y));
      for X := 0 to Width - 1 do
      begin
...
function FillRect(var Image: TImageData; X, Y, Width, Height: Integer; FillColor: Pointer): Boolean;
var
  I, J, ImageWidthBytes, RectWidthBytes: Integer;
...
begin
  ...
  try
    ...
    begin
      ImageWidthBytes := Image.Width;
      RectWidthBytes := Width;
      LinePointer := @PByteArray(Image.Bits)[Y * ImageWidthBytes + X];
...
begin
  ...
  try
    ...
    begin
      ...
      HasGlobalPal := FHeader.PackedFields and GIF_GlobalColorTable = GIF_GlobalColorTable;
      GlobalPalLength := FHeader.PackedFields and GIF_ColorTableSize;
...
procedure TGIFData.InternalRender(AIndex: Integer; var ABitmap: TBitmap);
var
...
begin
  Last := AIndex;
  First := Max(0, Last);
  UseCache := Assigned(FCachedBitmap) and (not FCachedBitmap.IsEmpty) and (FCachedIndex = AIndex - 1) and (FCachedIndex >= 0) and (FFrames[FCachedIndex].Disposal <> dmRestorePrevious);
  if UseCache then
  begin
    ABitmap.Assign(FCachedBitmap);
  end
  else begin
    FreeAndNil(FCachedBitmap);
  end;
  ...
  begin
    ...
    ABitmap.ClearRect(RectF(FFrames[FCachedIndex].Left, FFrames[FCachedIndex].Top, FFrames[FCachedIndex].Left + FFrames[FCachedIndex].Width, FFrames[FCachedIndex].Top + FFrames[FCachedIndex].Height), BGColor);
  end;
...
begin
  ...
  try
    AniBitmap.SetSize(Header.ScreenWidth, Header.ScreenHeight);
    AniBitmap.Clear(0);
    InternalRender(AFrameIndex, AniBitmap);
    ABitmap.Assign(AniBitmap);
    //ConvertImageDataToBitmap(AniFrame, ABitmap);
  finally
    FreeAndNil(AniBitmap);
  end;
end;
...
begin
  ...
  (
    ...
    begin
      ...
      FrameIndex := 0;
      AniFrame := TBitmap.Create;
      try
        AniFrame.SetSize(Header.ScreenWidth, Header.ScreenHeight);
